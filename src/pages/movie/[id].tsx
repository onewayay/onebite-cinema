  ...
  return (
    <div className={style.container}>
      <div
        ...
        style={{ backgroundImage: `url(${dummyData.posterImgUrl})` }}
      >
        <img src={dummyData.posterImgUrl} />
      </div>
      <div className={style.info_container}>
        <div>
          <h2>{dummyData.title}</h2>
          <div>
            {dummyData.releaseDate} / {dummyData.genres.join(', ')} /
            {dummyData.runtime}분
          </div>
          <div>{dummyData.company}</div>
        </div>
        <div>
          <div className={style.subTitle}>{dummyData.subTitle}</div>
          <div className={style.description}>{dummyData.description}</div>
        </div>
      </div>
    </div>
  );
